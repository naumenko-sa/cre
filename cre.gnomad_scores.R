...
gnomad_scores <- merge(gnomad_scores, genes_transcripts, by.x = "transcript",
                       by.y = "Ensembl_transcript_id", all.x = T, all.y = F)
...
colnames(gnomad_scores) <- c("Ensembl_gene_id", "Gnomad_oe_lof_score", "Gnomad_oe_mis_score",
                             "Exac_pli_score", "Exac_prec_score", "Exac_pnull_score")
