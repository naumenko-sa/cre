...
then
	callers="callers"
	caller_filter="Callers not in ('freebayes', 'samtools', 'platypus')"
else	
	callers="00"
	caller_filter=""
fi
...
sQuery=$sQuery" where gnomad_af_popmax <= "${max_af}" and "$caller_filter""${severity_filter}""
...
then
    ...
    cQuery=$initialQuery
    cQuery=$cQuery" where gnomad_af_popmax <= ${max_af} and "$caller_filter" and Clinvar <> ''"
    ...
    cQuery=$initialQuery
    cQuery=$cQuery" where gnomad_af_popmax > ${max_af} and "$caller_filter" and Clinvar_status != 'no_assertion_criteria_provided' and Clinvar in ('Pathogenic', 'Likely_pathogenic', 'Conflicting_interpretations_of_pathogenicity')"
