...
#		 (gt_types."$proband" == )"
    ...
    gemini query -q "$cQuery" --gt-filter "$s_gt_filter" $file
		...
		cQuery=$initialQuery
		cQuery=$cQuery" where gnomad_af_popmax > ${max_af} v.variant_id=i.variant_id and v.clinvar_status != 'no_assertion_criteria_provided' and Clinvar in ('Pathogenic', 'Likely_pathogenic', 'Conflicting_interpretations_of_pathogenicity')"
